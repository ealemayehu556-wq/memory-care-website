...
export default function Home() {
  ...
  const activeStyle = "bg-white text-blue-600 shadow-sm px-4 py-2 rounded-lg transition font-bold";
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => setActiveTab('home')}>
            {/* New Haven Care Logo Icon */}
            <div className="bg-blue-600 text-white font-bold p-2.5 rounded-2xl text-xl shadow-md">
              🛡️
            </div>
            {/* New Haven Care Logo Name */}
            <span className="text-2xl font-black tracking-tight text-slate-900">
              Haven<span className="text-blue-600"> Care</span>
            </span>
          </div>

          {/* Navigation Links */}
          <nav className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-xl text-sm font-semibold">
            <button
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6">
            ...
            <section className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-8 md:p-12 text-white shadow-xl">
              <div className="max-w-2xl space-y-4">
                <span className="bg-blue-500/30 text-blue-100 text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border border-blue-400/30">
                  Memory Care Portal • 2025
                </span>
                <h1 className="text-3xl md:text-5xl font-black tracking-tight leading-tight">
                  Welcome to Haven Care Support Portal
                </h1>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-emerald-50 border border-emerald-200 p-6 rounded-2xl space-y-2">
                ...
                <h3 className="font-bold text-emerald-900 text-lg">Video Call with Family</h3>
                <p className="text-xs text-emerald-700">Scheduled for 4:30 PM via Family Portal.</p>
              </div>
            </div>
          </div>
        )}         {/* CAREGIVERS SECTION (PRESERVED) */}
        ...
        {/* CONTACT SECTION (ADDIS ABABA DETAILS) */}
        {activeTab === 'contact' && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-slate-900">Contact & Support Center</h2>
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6 max-w-xl">
              <p className="text-sm text-slate-600">Reach out to our local team in Addis Ababa for caregiver support and program details.</p>
              
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl text-blue-900 space-y-1">
                  <div className="font-bold text-sm text-blue-700">📍 Office Location</div>
                  <div className="text-base font-semibold">Addis Ababa, Ethiopia</div>
                </div>

                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  <div className="font-bold text-sm text-slate-600">📞 Phone Contact</div>
                  <div className="text-base font-bold text-slate-900">+251 91 123 4567</div>
                </div>

                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  <div className="font-bold text-sm text-slate-600">✉️ Email Support</div>
                  <div className="text-base font-bold text-slate-900">contact@havencare.et</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-6 text-center text-xs text-slate-500">
        © 2025 Haven Care. All rights reserved.
      </footer>
    </div>
  );
}